...
exports.fromMask = void 0;
const assert_1 = require("../utils/assert");
const RoiMapManager_1 = require("./RoiMapManager");
...
function fromMask(mask, options = {}) {
    ...
    const MAX_ARRAY = constants_1.maxNumberRois - 1; // 65535 should be enough for most of the cases
    const maxPositiveId = constants_1.maxRoiId - 1;
    const maxNegativeId = -constants_1.maxRoiId;
    ...
    let positiveId = 0;
    let negativeId = 0;
    let columnToProcess = new Uint16Array(constants_1.maxNumberRois);
    let rowToProcess = new Uint16Array(constants_1.maxNumberRois);
    ...
    function analyseSurface(column, row) {
        ...
        while (from <= to) {
            ...
            from++;
            (0, assert_1.assert)(to - from <= MAX_ARRAY, 'fromMask can not finish, the array to manage internal data is not big enough.' +
                'You could improve mask by changing MAX_ARRAY');
        }
    }
    return new RoiMapManager_1.RoiMapManager({
        width: mask.width,
        height: mask.height,
        data,
