import { DirectConvolution, BorderType as ConvolutionBorderType, } from 'ml-convolution';
...
export function separableConvolution(image, kernelX, kernelY, options = {}) {
    const { normalize, borderType = BorderType.REFLECT_101, borderValue = 0, } = options;
    const interpolateBorder = getBorderInterpolation(borderType, borderValue);
    if (normalize) {
    ...
    const clamp = getClamp(newImage);
    const rowConvolution = new DirectConvolution(width, kernelX, ConvolutionBorderType.CUT);
    const columnConvolution = new DirectConvolution(height, kernelY, ConvolutionBorderType.CUT);
